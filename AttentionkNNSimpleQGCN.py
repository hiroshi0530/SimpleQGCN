...
class AttentionkNNSimpleQGCN(nn.Module):
    def __init__(self, config, dataset):
        ...
        q_adj_rows = row_idx.reshape(-1)
        q_adj_cols = idx.reshape(-1)
        q_adj_values = vals.reshape(-1)

        self.q_indices = torch.stack([torch.tensor(q_adj_rows), torch.tensor(q_adj_cols)], dim=0)
        self.q_values_tensor = torch.tensor(q_adj_values, dtype=torch.float32).to(self.device)

        ######################################
        size = (self.user_num + self.item_num, self.user_num + self.item_num)

        row_sums = torch.zeros(size[0], device=self.device, dtype=q_adj_values.dtype)
        row_sums.index_add_(0, self.q_indices[0], q_adj_values)  # 行ごとに和を集計

        row_sums[row_sums == 0] = 1.0

        self.q_values_tensor = (q_adj_values / row_sums[self.q_indices[0]]).to(self.device)

        ######################################
        self.w = torch.nn.Parameter(torch.rand(len(self.q_values_tensor), device=self.device, dtype=torch.float32))
    ...
    def forward(self):
        ...
        w = torch.sigmoid(self.get_w())

        self.q_norm_adj_matrix = torch.sparse_coo_tensor(
            self.q_indices,
            w * self.q_values_tensor,
            (self.user_num + self.item_num, self.user_num + self.item_num),
        )

        # # --- 3. 重複インデックスをまとめて正規化（重要） ---
        self.q_norm_adj_matrix = self.q_norm_adj_matrix.coalesce()

        a_mix = self.norm_adj_matrix + self.q_norm_adj_matrix
        a_mix = a_mix.coalesce()
