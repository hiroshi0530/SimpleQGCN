...
class SimpleQGCN_p(nn.Module):
    def __init__(self, config, dataset):
        ...
        ##################################################
        # power
        self.power_n = config["power_n"]
        self.Q0 = torch.pow(self.Q0 + 1, self.power_n) - 1

        # normalize Q0
        self.Q0 = self.Q0.to("cpu")  # to CPU
        D = torch.diag(torch.sum(self.Q0, dim=1)).type(torch.DoubleTensor)
        D_0_5_pow = torch.pow(torch.where(D == 0, 1, D), -1 / 2) - torch.where(D == 0, 1, 0)
        self.Q0 = D_0_5_pow.to(float) @ self.Q0.to(float) @ D_0_5_pow.to(float)

        self.Q0 = self.Q0.type(torch.FloatTensor).to(self.device)  # to GPU
